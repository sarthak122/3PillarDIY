...
const fetchData = async () => {
  try {
    ...
    countries.forEach((country) => {
      console.log(
        `country:${country.name} capital:${country.capital} languages:${country.languages} population${country.population}`
      );
    });
    ...
    console.log(
      `the total number of languages in the world is ${languages.length}`
    );
  } catch (err) {
    console.log(err);
  }
